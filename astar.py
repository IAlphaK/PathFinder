...
def astar_search(graph, start_node, goal_nodes):
    """
    Traverse the graph using the A* Search algorithm based on the graph generated without networkx library.
    Returns the path found by the A* Search algorithm and the sub-graph generated using networkx library.
    """
    visited = set()
    ...
    while priority_queue:
        ...
        for neighbor in neighbors:
            if neighbor not in visited:
                g_score = graph[current_node][neighbor]['weight']
                h_score = graph.nodes[neighbor].get('heuristic', 0)
                f_score = g_score + h_score
                priority_queue.append((neighbor, f_score))
